...
router.post('/:accountId/refresh', async (req, res) => {
  try {
    ...
    for (const plaidTx of transactions) {
      ...
      const datetime = plaidTx.datetime ? new Date(plaidTx.datetime) : new Date(txDate.setHours(12, 0, 0, 0));
...
router.delete('/:accountId', async (req, res) => {
    try {
      ...
      if (remainingAccounts === 0) {
        ...
        console.log(`${remainingAccounts} accounts still remaining for item ${itemId}`);
      }
